...
// ╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userDoc, (docSnap) => {
      ...
      if (isInitialLoad) {
        ...
        console.log('📥 Initial load:', { tasks: data.tasks?.length, timestamp: serverTimestamp });
      ...
      // אחרי הטעינה הראשונית - קבל עדכונים רק ממכשירים אחרים
      // אם ה-timestamp זהה לזה ששמרנו - זה העדכון שלנו, התעלם
      if (serverTimestamp === lastSaveTimestampRef.current) {
        console.log('⏭️ Ignoring own save echo');
        return;
      }
  ...
  useEffect(() => {
    ...
    // שמור אחרי 500ms של יציבות (debounce)
    pendingSaveRef.current = setTimeout(async () => {
      setIsSyncing(true);
      
      const timestamp = new Date().toISOString();
      // שמור את ה-timestamp לפני השמירה כדי לזהות את ה-echo
      ...
      try {
        ...
        // אם נכשל, אפס את ה-timestamp כדי לקבל עדכונים
