...
// Enable offline persistence (won't block real-time sync)
...
// ╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  const lastServerUpdateRef = useRef('');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userDoc, (docSnap) => {
      ...
      if (isInitialLoad) {
        isInitialLoad = false;
        lastServerUpdateRef.current = serverTimestamp;
        ...
        // טען הגדרות משתמש - מזג עם defaults
        ...
        setIsLoading(false);
        isFirstLoad.current = false;
        console.log('📥 Initial load complete:', { tasks: data.tasks?.length });
        return;
      }
      
      // עדכון ממכשיר אחר - רק אם זה לא השמירה שלנו
      if (!isSavingRef.current && serverTimestamp !== lastServerUpdateRef.current) {
        console.log('🔄 Sync from another device:', {
          serverTimestamp,
          lastKnown: lastServerUpdateRef.current
        });
  ...
  useEffect(() => {
    ...
    // שמור אחרי 800ms של יציבות (debounce)
    pendingSaveRef.current = setTimeout(async () => {
      ...
      try {
        ...
        lastServerUpdateRef.current = timestamp;
      ...
      // המתן קצת לפני שמאפשרים קבלת עדכונים
      setTimeout(() => {
